  ...
  template: `
    <div style="padding:40px;background:#0a1a10;min-height:100vh;color:#d4edda;font-family:'Inter',sans-serif">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:30px">
        <h1 style="color:#1a8c4e;margin:0">📊 Transactions</h1>
      ...
      <div *ngIf="!chargement() && transactions().length" style="display:flex;flex-direction:column;gap:8px">
        <div *ngFor="let t of transactions()" [routerLink]="['/transactions', t.id]" style="cursor:pointer" 
             style="background:#0f2d1a;border:1px solid rgba(26,140,78,0.15);border-radius:10px;padding:16px 20px;display:flex;justify-content:space-between;align-items:center">
          <div>
            <div style="font-family:monospace;color:#1a8c4e;font-size:13px">{{ t.referenceTransaction }}</div>
            <div style="font-size:12px;color:#6b9e74;margin-top:4px">{{ t.dateTransaction | date:'dd/MM/yyyy HH:mm' }} · {{ t.typeTransaction }} · {{ t.canal }}</div>
          </div>
      ...
      <div *ngIf="!chargement() && !transactions().length" style="text-align:center;padding:40px;color:#6b9e74">Aucune transaction trouvée</div>
    </div>
  `
})
